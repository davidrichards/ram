...
class PackagerTest < Test::Unit::TestCase
  ...
  def teardown
    Ram.set_template_namespace(nil)
    Ram.set_template_extension(nil)
    begin
  ...
  def test_fetching_lists_of_individual_urls
    ...
    urls = Ram.packager.individual_urls(:js_test_with_templates, :js)
  ...
  def test_fetching_lists_of_nested_urls
    urls = Ram.packager.individual_urls(:css_test_nested, :css)
  ...
  def test_packaging_javascripts
    ...
    js = Ram.packager.pack_javascripts(:js_test_with_templates)
  ...
  def test_package_caching
    css = Ram.packager.pack_stylesheets(:css_test, :mhtml, 'http://www.example.com')
    mtime = Time.now
    Ram.packager.cache(:css_test, :css, css, 'test/public', :mhtml, mtime)
  ...
  def test_precache_all
    ...
    Ram.packager.precache_all('test/precache', 'http://www.example.com')
  ...
  def test_precache_no_gzip
    Ram.load_configuration('test/config/assets-compression-disabled.yml').reload!
    Ram.packager.precache_all('test/precache', 'http://www.example.com')
  ...
  def test_exceptions_for_unwritable_directories
    ...
    assert_raises(OutputNotWritable) do
      Ram.packager.precache_all('test/fixtures/unwritable')
    end
  end

  def test_package_helper
    FileUtils.rm_rf("test/public/assets/*")
    Ram.package! :config_file => "test/config/assets.yml", :base_url => "http://example.com/"
  ...
  def test_packaging_javascripts_with_package_names
    FileUtils.rm_rf("test/public/assets/*")
    Ram.package! :config_file => "test/config/assets.yml", :package_names => [:js_test]
    assert File.exists?("test/public/assets/js_test.js")

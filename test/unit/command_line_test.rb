...
class CommandLineTest < Test::Unit::TestCase
  ...
  def test_version_and_help_can_run
    assert system('bin/ram -v') && system('bin/ram -h')
  end

  def test_ram_precaching
    system('bin/ram -c test/config/assets.yml -o test/precache -u http://www.example.com')
  ...
  def test_lazy_precaching
    ...
    system('bin/ram -c test/config/assets.yml -o test/precache -u http://www.example.com --force')
    assert File.mtime(PRECACHED_FILES.first) > mtime
  end
